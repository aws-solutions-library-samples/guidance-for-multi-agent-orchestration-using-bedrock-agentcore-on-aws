"""
Troubleshooting Agent Client Tool

Invokes the troubleshooting agent runtime for technical support queries.
Uses JWT authentication for agent-to-agent communication.
"""
...
@tool(context=True)
def troubleshooting_tool(query: str, tool_context: ToolContext) -> str:
    """
    Query technical documentation, troubleshooting guides, FAQs, and warranty information.
    
    Args:
        query: The troubleshooting query to send to the agent
        tool_context: Strands tool context for accessing agent state
        
    Returns:
        Response from the troubleshooting agent or structured error message
    """
    try:
        # Get customer_id and session_id from agent state
        customer_id = tool_context.agent.state.get("customer_id")
        if not customer_id:
            return "Unable to retrieve troubleshooting guidance: Customer ID not available"
        ...
        if not jwt_token:
            return "Unable to retrieve troubleshooting guidance: Authentication token not available"
        ...
        session_id = f"{base_session_id}-troubleshoot"
        ...
        if response.status_code == 200:
            ...
            return f"Unable to retrieve troubleshooting guidance: {error_msg}"
        ...
        return f"Unable to retrieve troubleshooting guidance: {str(e)}"
...
def _get_configuration() -> str:
    ...
    Returns:
        runtime_id: The troubleshooting agent runtime ID
    """
    ...
    param_name = os.environ.get('TROUBLESHOOTING_AGENT_RUNTIME_ID_PARAM')
    if not param_name:
        raise ValueError("TROUBLESHOOTING_AGENT_RUNTIME_ID_PARAM environment variable not set")
