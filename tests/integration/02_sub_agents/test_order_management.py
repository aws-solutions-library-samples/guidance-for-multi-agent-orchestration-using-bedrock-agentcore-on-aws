#!/usr/bin/env python3
"""
Order Management Agent Tests

Validates the Order Management agent can be invoked with JWT authentication
and successfully uses Gateway tools.
"""
...
def test_order_management_invocation(config, jwt_token):
    """Test 1: Invoke Order Management agent runtime with JWT."""
    print("\nTest 1: Order Management Runtime Invocation")
    print("-" * 60)
    
    session_id = f"test-order-{int(time.time())}"
    
    success, status, response = invoke_agentcore_runtime(
        runtime_arn=config['order_mgmt_runtime_arn'],
        jwt=jwt_token,
...
def test_order_management_tool_usage(config, jwt_token):
    """Test 2: Verify Order Management agent uses Gateway tools."""
    print("\nTest 2: Order Management Gateway Tool Usage")
    print("-" * 60)
    
    session_id = f"test-order-tools-{int(time.time())}"
    ...
    success, status, response = invoke_agentcore_runtime(
        ...
        customer_id=config['test_user']['customer_id'],
        session_id=session_id,
        query="Check my order status and inventory availability",
        region=config['aws_region']
    )
    ...
    runtime_name = config['order_mgmt_runtime_arn'].split('/')[-1]
