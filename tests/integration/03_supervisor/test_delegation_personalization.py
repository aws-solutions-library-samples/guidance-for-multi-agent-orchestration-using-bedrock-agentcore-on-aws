#!/usr/bin/env python3
"""
Supervisor Agent Personalization Delegation Tests

Validates the supervisor agent correctly delegates to the Personalization agent.
"""
...
def test_supervisor_delegates_to_personalization(config, jwt_token):
    """Test supervisor delegates customer preference queries to Personalization agent."""
    print("\nTest: Supervisor Delegation to Personalization")
    print("-" * 60)
    
    session_id = f"test-delegation-personalization-{int(time.time())}"
    start_time = datetime.utcnow()
    
    # Invoke supervisor with personalization-specific query
    success, status, response = invoke_agentcore_runtime(
        ...
        query="What are my preferences and browsing history?",
        region=config['aws_region'],
        timeout=60
    )
    ...
    delegation_found, analysis = check_delegation_in_logs(
        ...
        region=config['aws_region'],
        session_id=session_id,
        target_runtime_name="personalization"
    )
    ...
    print("\n✓ Delegation to Personalization verified")
    print("✓ PASSED")
