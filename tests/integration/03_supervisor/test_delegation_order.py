#!/usr/bin/env python3
"""
Supervisor Agent Order Management Delegation Tests

Validates the supervisor agent correctly delegates to the Order Management agent.
"""
...
def test_supervisor_delegates_to_order_management(config, jwt_token):
    """Test supervisor delegates order-related queries to Order Management agent."""
    print("\nTest: Supervisor Delegation to Order Management")
    print("-" * 60)
    
    session_id = f"test-delegation-order-{int(time.time())}-session"
    start_time = datetime.utcnow() - timedelta(minutes=1)  # 1 minute buffer to catch all logs
    
    # Invoke supervisor with order-specific query
    success, status, response = invoke_agentcore_runtime(
        ...
        query="Show me my order history and current order status",
        region=config['aws_region'],
        timeout=60
    )
    ...
    print("\n✓ Delegation to Order Management verified")
    print("✓ PASSED")
