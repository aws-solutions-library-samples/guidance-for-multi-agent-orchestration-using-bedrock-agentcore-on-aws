#!/usr/bin/env python3
"""
Supervisor Agent Troubleshooting Delegation Tests

Validates the supervisor agent correctly delegates to the Troubleshooting agent.
"""
...
def test_supervisor_delegates_to_troubleshooting(config, jwt_token):
    """Test supervisor delegates help queries to Troubleshooting agent."""
    print("\nTest: Supervisor Delegation to Troubleshooting")
    print("-" * 60)
    
    session_id = f"test-delegation-troubleshooting-{int(time.time())}"
    start_time = datetime.utcnow()
    
    # Invoke supervisor with troubleshooting-specific query
    success, status, response = invoke_agentcore_runtime(
        ...
        query="I need help with my account. How do I reset my password?",
        region=config['aws_region'],
        timeout=60
    )
    ...
    # Extract runtime name from ARN and construct log group
    ...
    delegation_found, analysis = check_delegation_in_logs(
        ...
        region=config['aws_region'],
        session_id=session_id,
        target_runtime_name="troubleshooting"
    )
    ...
    print("\n✓ Delegation to Troubleshooting verified")
    print("✓ PASSED")
